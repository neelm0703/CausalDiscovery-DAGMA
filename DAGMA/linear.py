...
class DagmaLinear:
    ...
    def minimize(self, 
        ...
        for iter in range(1, max_iter+1):
            ...
            if iter % self.checkpoint == 0 or iter == max_iter:
                W_est = W.copy()
                W_est = (np.abs(W_est) > 0.3).astype(int)
                res = calculate_metrics(W_est, self.W_true, False)
                self.cimcc_history.append(res[1])
                self.shd_history.append(res[0])
                obj_new, score, h = self._func(W, mu, s)
    ...
    def fit(self, 
        ...
        self.W_true = df_W_true.to_numpy()
...
def test():
    ...
    # W_est = model.fit(X, lambda1=0.02, warm_iter = 3e2, max_iter = 6e2)
    ...
    plt.title("DAGMA Training Curve")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("dagma_linear_training_loss.png", dpi=300)

    nodes = df.columns
    W_df = pd.DataFrame(W_est, index=nodes, columns=nodes)
    W_df.to_csv("DAGMA_linear_W_est.csv")        
